...
class FormInteractionAgent:
    ...
    async def propose_action(
        ...
        target_fid = top_update.field_id if top_update else str(
        ...
        is_preference = (
            mapping.get("intent") == "update_preferences"
            or mapping.get("action_type") == "preference"
        )
        ...
        # confirmation gates.
        if profile.confirm_all_actions and act_type not in {"type", "select", "click", "focus", "preference"}:
            is_high_risk = True
        ...
        if not updates and not is_submission and not is_reset and not is_preference:
        ...
        proposal = FormActionProposal(
            ...
            preferences=dict(mapping.get("preferences", {}) or {}),
