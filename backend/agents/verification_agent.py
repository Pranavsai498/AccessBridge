...
class VerificationAgent:
    ...
    async def verify(
        ...
        if action.action_type == "preference" or action.intent == "update_preferences":
            result = VerificationResult(
                field_match=1.0,
                semantic_match=max(0.0, min(1.0, action.confidence)),
                format_valid=True,
                ambiguity=False,
                is_reversible=True,
                is_high_risk=False,
                notes="Accessibility preference change; no web form field is touched.",
            )
