...
"""
    ...
    def _fallback_intent(user_message: str) -> Dict[str, Any]:
        ...
        if any(token in message for token in ["submit", "send the application", "finish the application"]):
            goal = "form_submission"
        elif any(token in message for token in ["reset", "start over", "clear the form"]):
            goal = "form_reset"
        elif re.match(r"^https?://", message):
            goal = "website_navigation"
        else:
            goal = "form_entry"
        return {
            "goal": goal,
    ...
    def _fallback_map_user_answer(
        ...
        """
        Small emergency fallback. It is intentionally limited; semantic
        understanding belongs to Gemini whenever Gemini is available.
        """
        message = user_message.strip()
        lowered = message.lower()

        if any(token in lowered for token in ["submit application", "submit the form", "send the application"]):
            return {
                "intent": "submit",
                "field_updates": [],
                "action_type": "submit",
                ...
                "needs_clarification": False,
                "is_high_risk": True,
                "reasoning": "Fallback detected an explicit submission request.",
                "source": "deterministic_fallback",
            }

        if any(token in lowered for token in ["reset form", "start over", "clear the form"]):
            return {
                "intent": "reset",
                "field_updates": [],
                "action_type": "clear",
                ...
                "needs_clarification": False,
                "is_high_risk": True,
                "reasoning": "Fallback detected an explicit reset request.",
                "source": "deterministic_fallback",
            }
        ...
        if email_match:
            for field in fields:
                if field.type == "email" or "email" in field.semantic_name.lower() or "email" in field.label.lower():
                    updates.append({
                        "field_id": field.field_id,
                        "value": email_match.group(0),
                        "action_type": "type",
                        "confidence": 0.95,
                        "reasoning": "Fallback extracted an email address.",
                        "is_ambiguous": False,
                        "is_high_risk": False,
                    })
                    break

        name_match = re.search(r"(?i)(?:my\s+(?:full\s+|legal\s+)?name\s+is|i\s+am)\s+([^,.!?]+)", message)
        if name_match:
            for field in fields:
                if "name" in (field.semantic_name + " " + field.label).lower() and field.type in {"text", "textarea"}:
                    updates.append({
                        "field_id": field.field_id,
                        "value": name_match.group(1).strip(),
                        "action_type": "type",
                        "confidence": 0.90,
                        "reasoning": "Fallback extracted a name.",
                        "is_ambiguous": False,
                        "is_high_risk": False,
                    })
                    break
        ...
        if not updates and current_field_id:
            field = next((f for f in fields if f.field_id == current_field_id), None)
            if field:
                action_type = "select" if field.type in {"select", "radio"} else "type"
                if field.type in {"select", "radio"} and field.options:
                    exact = next((o for o in field.options if o.label.lower() == lowered or o.value.lower() == lowered), None)
                    if exact:
                        updates.append({
                            "field_id": field.field_id,
                            "value": exact.value,
                            "action_type": "select" if field.type == "select" else "click",
                            "confidence": 0.78,
                            "reasoning": "Fallback matched the current option.",
                            "is_ambiguous": False,
                            "is_high_risk": False,
                        })
                else:
                    updates.append({
                        ...
                        "action_type": action_type,
                        "confidence": 0.60,
                        "reasoning": "Fallback used the currently active field.",
                        "is_ambiguous": False,
                        "is_high_risk": False,
                    })
        ...
        return {
            ...
            "field_updates": updates,
            "action_type": "fill" if updates else "unknown",
            "confidence": min([u["confidence"] for u in updates], default=0.30),
            "is_ambiguous": not bool(updates),
            "needs_clarification": not bool(updates),
            "is_high_risk": False,
            "reasoning": "Deterministic fallback used because Gemini was unavailable or invalid.",
            "source": "deterministic_fallback",
        }
