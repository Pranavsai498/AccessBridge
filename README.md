...
17. [Browser Extension](#browser-extension-use-accessbridge-on-any-website)
18. [Running Instructions & Launch Commands](#running-instructions--launch-commands)
19. [3-Minute Demo Walkthrough](#3-minute-demo-walkthrough)
20. [Hackathon Requirement Mapping](#hackathon-requirement-mapping)
...
## Browser Extension (Use AccessBridge on Any Website)

AccessBridge ships as a **Manifest V3 browser extension** so the assistant is no
longer limited to the bundled demo form. It opens in a side panel next to the
page the user is already on, reads that page's form controls, and applies only
the actions the confidence gate approved.

### Install
1. Start the backend (`python run_server.py`, serving `http://127.0.0.1:8000`).
2. Open `chrome://extensions` (Chrome, Edge, Brave, Arc or Opera).
3. Enable **Developer mode**, click **Load unpacked**, and select the
   `extension/` folder — or unzip `accessbridge-extension.zip` first.
4. Pin **AccessBridge Assistant** and click it on any page containing a form.

### Capabilities
- Plain-language and voice form filling on third-party websites.
- Spoken replies, high-contrast mode and larger text applied to the live page.
- Explicit confirmation before any submission; injection attempts in page text
  are ignored and surfaced to the user.
- Password, card, CVV, SSN, PIN and account-number fields are never read.

### Architecture
The extension posts a page snapshot to `POST /api/extension/assist`. That
endpoint runs the **same** LangGraph pipeline (intake → profile → web
understanding → WCAG RAG → proposal → verification → safety gate) against a
`RemoteDOMEngine` (`backend/browser/remote_engine.py`), which *records* actions
instead of performing them. The approved action list is returned to the side
panel, and the content script is the only component that touches the DOM — the
server never drives the user's browser.
